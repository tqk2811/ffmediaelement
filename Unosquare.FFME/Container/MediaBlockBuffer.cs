namespace Unosquare.FFME.Container
{
    ...
    internal sealed class MediaBlockBuffer : IDisposable
    {
        ...
        internal MediaBlock Add(MediaFrame source, MediaContainer container)
        {
            ...
            lock (SyncLock)
            {
                try
                {
                    ...
                    var lastBlock = PlaybackBlocks.LastOrDefault(); // PlaybackBlocks.Count > 0 ? PlaybackBlocks[^1] : null;
        ...
        private void UpdateCollectionProperties()
        {
            ...
            m_RangeEndTime = PlaybackBlocks.Count == 0 ? TimeSpan.Zero : PlaybackBlocks.Last().EndTime; // PlaybackBlocks[^1].EndTime;
